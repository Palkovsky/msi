...
fn test_input(
    ...
    assert_eq!(input.call(20.0)?.remove("very quiet"), Some(0.5));
    assert_eq!(input.call(20.0)?.remove("quiet"), Some(0.5));
    assert_eq!(input.call(30.0)?.remove("very quiet"), Some(0.0));
    assert_eq!(input.call(-30.0)?.remove("very quiet"), Some(1.0));
    assert_eq!(input.call(130.0)?.remove("very quiet"), Some(0.0));
    assert_eq!(input.call(130.0)?.remove("very loud"), Some(1.0));
    assert_eq!(input.call(130.0)?.remove("loud"), Some(0.0));
    assert_eq!(input.call(90.0)?.remove("loud"), Some(0.0));
    assert_eq!(input.call(90.0)?.remove("very loud"), Some(1.0));
    assert_eq!(input.call(25.0)?.remove("very quiet"), Some(0.25));

    plot::set(&input, "Loudness", "Y").to_svg("test1.svg")
}
...
fn test_fuzzy(
    ...
    let values = values! {
        "loudness" => 50.0;
        String::from("tod") => 12.0
    };

    let change = fuzzer.apply(&values)?.remove("change").unwrap();
    let cog = defuzz::cog(change.points("out")?)?;
    plot::set(&change, format!("COG: {}", cog), "Y").to_svg("test3.svg")?;
